...
export async function append_home_page(url, store_id, append_to, redirect_fn) {
    const home_route = await get(
        store_id,
        'web.default.cms_home_page',
        'magento 2 home route'
    );
    ...
    if (home_route) {
        ...
        if (
            ...
            url.match(
                new RegExp(
                    `^/(?:${Object.keys(append_to.stores).join('|')})+/$`
                )
            )
        ) {
            ...
            append_to._wyvr.template = ['shop/HomePage', 'shop/Default'].concat(
                append_to._wyvr.template || []
            );
