import { execute_page, get_config } from '@wyvr/generator/cron.js';
...
export async function clear_all_urls(index) {
    if (!filled_string(index)) {
        logger.error('no elasticsearch index was provided');
        return;
    }
    ...
    if (total_routes && total_routes.length > 0) {
        ...
        logger.info('no routes to clear');
    }
...
export async function clear_urls(index, data_docs) {
    ...
    if (!filled_array(data_docs)) {
        logger.error('no elasticsearch clear data was provided');
        return;
    }
        ...
        for (let i = 0, len = chunks.length; i < len; i++) {
            ...
            await Promise.all(
                chunk.map(async (url) => {
                    ...
                    if (error) {
                        logger.error('error triggering', url, error);
                    }
                    return ok;
                })
            );
