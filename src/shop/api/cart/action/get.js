...
export async function get_cart_action(
    store,
    email_or_cart_token,
    bearer_token,
    is_prod = false
    ...
    if (is_guest) {
        const [create_guest_cart_error, guest_cart_meta] =
            await create_guest_cart(store, email_or_cart_token, is_prod);
        ...
        if (get_guest_cart_error) {
            ...
            const [new_create_guest_cart_error, new_guest_cart_meta] =
                await create_guest_cart(store, 'guest', is_prod);
            if (new_create_guest_cart_error) {
                return end(get_guest_cart_error, undefined);
            }

            const [new_get_guest_cart_error, new_guest_cart] =
                await get_guest_cart(
                    store,
                    new_guest_cart_meta?.cart_id,
                    is_prod
                );
            if (new_guest_cart) {
                logger.success(
                    'magento2 get cart, heal guest cart',
                    email_or_cart_token,
                    'to',
                    new_guest_cart_meta?.cart_id
                );
    ...
    const [create_cart_error, cart_meta] = await create_cart(
        store,
        email_or_cart_token,
        base_customer?.id,
        is_prod
    ...
    const [get_cart_error, cart] = await get_cart(
        store,
        cart_meta?.id,
        is_prod
    );
