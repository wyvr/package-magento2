    ...
    onExec: async ({ body, params, returnJSON, headers, isProd }) => {
        const body_is_valid =
            ...
            Object.entries(body).every(([sku, qty]) => {
                return typeof sku === 'string' && sku && typeof qty === 'number';
            });
        if (!body_is_valid) {
        ...
        const updated_cart_result = await update_cart_action(params.store, params.email, cart_result.cart, body, isProd);
        if (updated_cart_result.error) {
            return returnJSON({ message: updated_cart_result.error }, updated_cart_result.status);
        }
