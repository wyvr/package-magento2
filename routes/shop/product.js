...
export default {
    ...
    onExec: async ({ params, query, data, setStatus, returnRedirect, returnJSON, isProd }) => {
        ...
        if (redirect_simple_to_configurable && product.type_id === 'simple' && Array.isArray(product.parent_products) && product.parent_products.length > 0) {
    ...
    _wyvr: ({ data }) => {
        ...
        return {
            ...(data?._wyvr ?? {}),
            ...{
                template: [`shop/product/id/${data.product.entity_id}`, `shop/product/sku/${sku}`, `shop/product/${type}`, 'shop/Product', 'shop/Default'],
                methods: ['get'],
                persist: true
            }
        };
